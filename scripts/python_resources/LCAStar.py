...
class LCAStar(object):
    ...
    def __init__(self, filename, megan_names_file=None):
        ...
        self.megan_names = None # hash of NCBI ID to preferred megan names if available
        ...
        if megan_names_file:
            self.megan_names = {}
            fh = open(megan_names_file, 'r')
            lines = fh.readlines()
            fh.close()
            
            for line in lines:
                fields = line.split("\t")
                if fields[0] not in self.megan_names:
                    self.megan_names[fields[0]] = fields[1]
    ...
    def translateIdToName(self, id):
       if self.megan_names:
           if id in self.megan_names:
               return self.megan_names[id]
    ...
    def get_lca(self, IDs, print_id=False):
        limit = len(IDs)
        for id in IDs:
           tid = id 
    ...
    def getTaxonomy(self, name_groups, print_id=False):
         IDs = []
         for name_group in name_groups:
            id = self.get_a_Valid_ID(name_group)
            if id!=-1:
              IDs.append(id)
         consensus = self.get_lca(IDs, print_id)
    ...
    def lca_star(self, taxalist, p_value=False, print_id=False):
        ...
        if majority != None:
           ...
           if print_id:
               return (self.get_a_Valid_ID([majority]), str(p_val))
           else:
